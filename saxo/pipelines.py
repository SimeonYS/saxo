...
class SaxoPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('saxo.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `saxo`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM saxo WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO saxo (date, title, link, content)"
